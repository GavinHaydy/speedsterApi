...
var (
	...
	roleRowsWithPlaceHolder = builder.PostgreSqlJoin(stringx.Remove(roleFieldNames, "id", "create_at", "create_time", "created_at", "update_at", "update_time", "updated_at"))
)
...
func (m *defaultRoleModel) FindOneByCode(ctx context.Context, code string) (*Role, error) {
	var resp Role
	query := fmt.Sprintf("select %s from %s where code = $1 limit 1", roleRows, m.table)
...
func (m *defaultRoleModel) Insert(ctx context.Context, data *Role) (sql.Result, error) {
	query := fmt.Sprintf("insert into %s (%s) values ($1, $2, $3, $4)", m.table, roleRowsExpectAutoSet)
	ret, err := m.conn.ExecCtx(ctx, query, data.Name, data.Code, data.Description, data.Status)
	return ret, err
}
...
func (m *defaultRoleModel) Update(ctx context.Context, newData *Role) error {
	...
	_, err := m.conn.ExecCtx(ctx, query, newData.Id, newData.Name, newData.Code, newData.Description, newData.Status)
	return err
}
